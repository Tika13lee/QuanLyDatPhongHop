...
export default function ScheduleDetailRoom({ navigation, route }) {
  const { reservationId: reservationData, employeeId, timeCheckIn } = route.params;
  ...
  const fetchSchedule = async () => {
    try {
      ...
      const timeClock = getTimeDifference(timeCheckIn, res.data.timeEnd);
      setClock({
  ...
  return (
    <DefaultLayout>
      ...
      <Popup
        ...
        handleOnPopup={(type) => {
          if (type == "Hủy") {
            setIsOpenPopup((prev) => ({ success: false, warning: false }));
          } else if (type == "OK") {
            setIsOpenPopup((prev) => ({ success: false, warning: false }));
      ...
      <Popup
        ...
        status={"success"}
        handleOnPopup={(type) => {
          setIsOpenPopup((prev) => ({ warning: false, success: false }));
          navigation.navigate("Tabs");
        }}
